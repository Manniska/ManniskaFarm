...
local THEMES = {
    ...
    ["Cyber Purple"] = {
        Background = Color3.fromRGB(16, 12, 24), Surface = Color3.fromRGB(26, 20, 38), Border = Color3.fromRGB(60, 42, 88), Accent = Color3.fromRGB(168, 85, 247), RecordActive = Color3.fromRGB(240, 70, 70), PlayActive = Color3.fromRGB(46, 204, 113), ToggleOff = Color3.fromRGB(40, 32, 58), TextPrimary = Color3.fromRGB(248, 245, 252), TextSecondary = Color3.fromRGB(165, 150, 185), GradientEnabled = true, GradientColor = Color3.fromRGB(90, 20, 140)
    },
    ["Emerald"] = {
        Background = Color3.fromRGB(10, 18, 14), Surface = Color3.fromRGB(18, 28, 22), Border = Color3.fromRGB(34, 60, 44), Accent = Color3.fromRGB(16, 185, 129), RecordActive = Color3.fromRGB(240, 70, 70), PlayActive = Color3.fromRGB(16, 185, 129), ToggleOff = Color3.fromRGB(26, 42, 34), TextPrimary = Color3.fromRGB(240, 248, 244), TextSecondary = Color3.fromRGB(140, 170, 155), GradientEnabled = true, GradientColor = Color3.fromRGB(10, 80, 50)
    },
    ["Crimson Sunset"] = {
        Background = Color3.fromRGB(20, 12, 16), Surface = Color3.fromRGB(30, 18, 24), Border = Color3.fromRGB(70, 32, 45), Accent = Color3.fromRGB(244, 63, 94), RecordActive = Color3.fromRGB(240, 60, 80), PlayActive = Color3.fromRGB(46, 204, 113), ToggleOff = Color3.fromRGB(46, 26, 34), TextPrimary = Color3.fromRGB(252, 242, 244), TextSecondary = Color3.fromRGB(185, 145, 155), GradientEnabled = true, GradientColor = Color3.fromRGB(140, 20, 40)
    },
    ["Solar Amber"] = {
        Background = Color3.fromRGB(20, 16, 10), Surface = Color3.fromRGB(32, 24, 16), Border = Color3.fromRGB(68, 52, 32), Accent = Color3.fromRGB(245, 158, 11), RecordActive = Color3.fromRGB(240, 70, 70), PlayActive = Color3.fromRGB(46, 204, 113), ToggleOff = Color3.fromRGB(46, 36, 24), TextPrimary = Color3.fromRGB(252, 248, 240), TextSecondary = Color3.fromRGB(185, 168, 145), GradientEnabled = true, GradientColor = Color3.fromRGB(120, 65, 10)
    }
}
...
local Config = {
    ...
    RecordMouseClicks = false -- NEW ADDITION
}
...
local function playSoundFeedback(pitch)
    pcall(function()
        ...
        sound.SoundId = "rbxasset://sounds/button.wav"; sound.Volume = 0.4; sound.PlaybackSpeed = pitch or 1.0; sound.Parent = workspace; sound:Play()
...
local function setupAntiAFK()
    local idledConn = player.Idled:Connect(function()
        if Config.AntiAFKEnabled then VirtualUser:CaptureController(); VirtualUser:ClickButton2(Vector2.zero) end
    end)
    table.insert(scriptConnections, idledConn)
end
setupAntiAFK()

-- FAST CHARACTER RETRIEVAL (Fixes 30-Second Timeout Freeze)
local function getCharacter()
    local char = player.Character
    if not char then return nil, nil, nil end
    local hum = char:FindFirstChildWhichIsA("Humanoid")
    local root = char.PrimaryPart or char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso")
...
screenGui.Name = "ManniskaFarmUI"; screenGui.ResetOnSpawn = false; screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; screenGui.Parent = playerGui
...
-- ADORNMENT VISUALIZER SETUP (Fixes Node Lag & Memory Issues)
local visualizerFolder = workspace:FindFirstChild("ManniskaPathVisualizer") or Instance.new("Folder")
visualizerFolder.Name = "ManniskaPathVisualizer"; visualizerFolder.Parent = workspace

local visualizerAnchor = workspace:FindFirstChild("ManniskaVisAnchor") or Instance.new("Part")
visualizerAnchor.Name = "ManniskaVisAnchor"; visualizerAnchor.Anchored = true; visualizerAnchor.CanCollide = false
visualizerAnchor.Transparency = 1; visualizerAnchor.Position = Vector3.zero; visualizerAnchor.Parent = workspace

local activeNodeHalo = Instance.new("Part")
activeNodeHalo.Name = "ActiveTargetHalo"; activeNodeHalo.Shape = Enum.PartType.Cylinder; activeNodeHalo.Size = Vector3.new(0.3, 3, 3)
activeNodeHalo.Anchored = true; activeNodeHalo.CanCollide = false; activeNodeHalo.Material = Enum.Material.Neon; activeNodeHalo.Color = Color3.fromRGB(255, 230, 0)
activeNodeHalo.Transparency = 1; activeNodeHalo.CastShadow = false; activeNodeHalo.Parent = visualizerFolder
...
local function getVisNode(idx)
    ...
    visualizerPool.Nodes[idx] = sphere; return sphere
end
...
local function getVisBeam(idx)
    ...
    visualizerPool.Beams[idx] = line; return line
end
...
local function getVisLabel(idx)
    ...
    billboard.Adornee = attach; billboard.Size = UDim2.new(0, 48, 0, 18); billboard.StudsOffset = Vector3.new(0, 1.2, 0); billboard.AlwaysOnTop = true; billboard.Parent = visualizerFolder
    local tag = Instance.new("TextLabel")
    tag.Size = UDim2.new(1, 0, 1, 0); tag.BackgroundTransparency = 1; tag.TextSize = 10; tag.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); tag.Parent = billboard
    local lblData = { Billboard = billboard, Tag = tag, Attach = attach }
    visualizerPool.Labels[idx] = lblData; return lblData
end
...
setHaloTarget = function(pos)
    if not Config.VisualizerEnabled or not pos then activeNodeHalo.Transparency = 1; return end
    activeNodeHalo.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
    activeNodeHalo.Transparency = 0.3
end

-- Scoped UI Construction (Identical to V12.4)
do
    ...
    hudBadge = Instance.new("Frame"); hudBadge.Name = "StatusBadge"; hudBadge.Size = UDim2.new(0, 48, 0, 20); hudBadge.Position = UDim2.new(0, 10, 0.5, -10); hudBadge.BackgroundColor3 = Color3.fromRGB(60, 65, 80); hudBadge.BorderSizePixel = 0; hudBadge.Parent = statusHUD
    local hudBadgeCorner = Instance.new("UICorner"); hudBadgeCorner.CornerRadius = UDim.new(0, 4); hudBadgeCorner.Parent = hudBadge
    hudBadgeLabel = Instance.new("TextLabel"); hudBadgeLabel.Size = UDim2.new(1, 0, 1, 0); hudBadgeLabel.BackgroundTransparency = 1; hudBadgeLabel.Text = "IDLE"; hudBadgeLabel.TextColor3 = Color3.fromRGB(240, 240, 245); hudBadgeLabel.TextSize = 9; hudBadgeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); hudBadgeLabel.Parent = hudBadge

    hudNodeLabel = Instance.new("TextLabel"); hudNodeLabel.Size = UDim2.new(0, 110, 1, 0); hudNodeLabel.Position = UDim2.new(0, 66, 0, 0); hudNodeLabel.BackgroundTransparency = 1; hudNodeLabel.Text = "Node: 0/0"; hudNodeLabel.TextColor3 = activeTheme.TextPrimary; hudNodeLabel.TextSize = 11; hudNodeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); hudNodeLabel.TextXAlignment = Enum.TextXAlignment.Left; hudNodeLabel.Parent = statusHUD
    registerElement("TextPrimary", hudNodeLabel, "TextColor3")

    hudLoopLabel = Instance.new("TextLabel"); hudLoopLabel.Size = UDim2.new(0, 120, 1, 0); hudLoopLabel.Position = UDim2.new(1, -130, 0, 0); hudLoopLabel.BackgroundTransparency = 1; hudLoopLabel.Text = "Loop: 0/Inf | 1.0x"; hudLoopLabel.TextColor3 = activeTheme.Accent; hudLoopLabel.TextSize = 11; hudLoopLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); hudLoopLabel.TextXAlignment = Enum.TextXAlignment.Right; hudLoopLabel.Parent = statusHUD
    ...
    mainFrame = Instance.new("Frame"); mainFrame.Name = "MainWindow"; mainFrame.Size = UDim2.new(0, 440, 0, 520); mainFrame.Position = UDim2.new(0.5, -220, 0.32, -260); mainFrame.BackgroundColor3 = activeTheme.Background; mainFrame.BackgroundTransparency = 0.15; mainFrame.BorderSizePixel = 0; mainFrame.ClipsDescendants = true; mainFrame.Parent = screenGui
    ...
    bgGradient = Instance.new("UIGradient"); bgGradient.Rotation = 45; bgGradient.Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, activeTheme.Background), ColorSequenceKeypoint.new(1, activeTheme.GradientColor or activeTheme.Background) })
    bgGradient.Enabled = (activeTheme.GradientEnabled == true); bgGradient.Parent = mainFrame

    local header = Instance.new("Frame"); header.Name = "Header"; header.Size = UDim2.new(1, 0, 0, 48); header.BackgroundColor3 = activeTheme.Surface; header.BackgroundTransparency = 0.2; header.BorderSizePixel = 0; header.Parent = mainFrame
    ...
    local titleGroup = Instance.new("Frame"); titleGroup.Size = UDim2.new(1, -110, 1, 0); titleGroup.BackgroundTransparency = 1; titleGroup.Parent = header
    ...
    local titleTopRow = Instance.new("Frame"); titleTopRow.Size = UDim2.new(1, 0, 0, 16); titleTopRow.BackgroundTransparency = 1; titleTopRow.Parent = titleGroup
    ...
    local titleLabel = Instance.new("TextLabel"); titleLabel.Name = "Title"; titleLabel.Size = UDim2.new(0, 145, 1, 0); titleLabel.BackgroundTransparency = 1; titleLabel.Text = "MANNISKAFARM V12.6"; titleLabel.TextColor3 = activeTheme.TextPrimary; titleLabel.TextSize = 13; titleLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); titleLabel.TextXAlignment = Enum.TextXAlignment.Left; titleLabel.Parent = titleTopRow
    registerElement("TextPrimary", titleLabel, "TextColor3")

    local stateBadge = Instance.new("Frame"); stateBadge.Name = "StateBadge"; stateBadge.Size = UDim2.new(0, 56, 0, 16); stateBadge.BackgroundColor3 = Color3.fromRGB(60, 65, 80); stateBadge.BackgroundTransparency = 0.2; stateBadge.BorderSizePixel = 0; stateBadge.Parent = titleTopRow
    ...
    local stateBadgeLabel = Instance.new("TextLabel"); stateBadgeLabel.Size = UDim2.new(1, 0, 1, 0); stateBadgeLabel.BackgroundTransparency = 1; stateBadgeLabel.Text = "IDLE"; stateBadgeLabel.TextColor3 = Color3.fromRGB(220, 225, 235); stateBadgeLabel.TextSize = 9; stateBadgeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); stateBadgeLabel.Parent = stateBadge

    updateStateBadge = function(stateName, color)
        stateBadgeLabel.Text = stateName; hudBadgeLabel.Text = stateName
    ...
    local subLabel = Instance.new("TextLabel"); subLabel.Name = "SubTitle"; subLabel.Size = UDim2.new(1, 0, 0, 12); subLabel.BackgroundTransparency = 1; subLabel.Text = "HARDWARE BRIDGE & INTERACTION SUITE"; subLabel.TextColor3 = activeTheme.Accent; subLabel.TextSize = 10; subLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); subLabel.TextXAlignment = Enum.TextXAlignment.Left; subLabel.Parent = titleGroup
    registerElement("AccentText", subLabel, "TextColor3")

    local headerBtnGroup = Instance.new("Frame"); headerBtnGroup.Size = UDim2.new(0, 64, 1, 0); headerBtnGroup.Position = UDim2.new(1, -64, 0, 0); headerBtnGroup.BackgroundTransparency = 1; headerBtnGroup.Parent = header
    ...
    local isMinimized = false; local unminimizedHeight = 520
    local minBtn = Instance.new("TextButton"); minBtn.Name = "MinButton"; minBtn.Size = UDim2.new(0, 28, 0, 28); minBtn.BackgroundColor3 = activeTheme.Surface; minBtn.BackgroundTransparency = 0.6; minBtn.BorderSizePixel = 0; minBtn.AutoButtonColor = false; minBtn.Text = "—"; minBtn.TextColor3 = activeTheme.TextSecondary; minBtn.TextSize = 13; minBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); minBtn.Parent = headerBtnGroup
    registerElement("Surface", minBtn, "BackgroundColor3"); registerElement("TextSecondary", minBtn, "TextColor3")
    local minCorner = Instance.new("UICorner"); minCorner.CornerRadius = UDim.new(0, 6); minCorner.Parent = minBtn

    local closeBtn = Instance.new("TextButton"); closeBtn.Name = "CloseButton"; closeBtn.Size = UDim2.new(0, 28, 0, 28); closeBtn.BackgroundColor3 = activeTheme.Surface; closeBtn.BackgroundTransparency = 0.6; closeBtn.BorderSizePixel = 0; closeBtn.AutoButtonColor = false; closeBtn.Text = "×"; closeBtn.TextColor3 = activeTheme.TextSecondary; closeBtn.TextSize = 20; closeBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular); closeBtn.Parent = headerBtnGroup
    registerElement("Surface", closeBtn, "BackgroundColor3"); registerElement("TextSecondary", closeBtn, "TextColor3")
    local closeCorner = Instance.new("UICorner"); closeCorner.CornerRadius = UDim.new(0, 6); closeCorner.Parent = closeBtn

    minBtn.MouseButton1Click:Connect(function()
        isMinimized = not isMinimized
        if isMinimized then unminimizedHeight = mainFrame.AbsoluteSize.Y; TweenService:Create(mainFrame, TWEEN_QUICK, { Size = UDim2.new(0, mainFrame.AbsoluteSize.X, 0, 48) }):Play()
        else TweenService:Create(mainFrame, TWEEN_QUICK, { Size = UDim2.new(0, mainFrame.AbsoluteSize.X, 0, unminimizedHeight) }):Play() end
    end)
    ...
    closeBtn.MouseEnter:Connect(function() TweenService:Create(closeBtn, TWEEN_QUICK, { BackgroundColor3 = Color3.fromRGB(235, 75, 75), BackgroundTransparency = 0, TextColor3 = Color3.fromRGB(255, 255, 255) }):Play() end)
    closeBtn.MouseLeave:Connect(function() TweenService:Create(closeBtn, TWEEN_QUICK, { BackgroundColor3 = activeTheme.Surface, BackgroundTransparency = 0.6, TextColor3 = activeTheme.TextSecondary }):Play() end)
    closeBtn.MouseButton1Click:Connect(function() setGuiVisible(false) end)

    local progressTrack = Instance.new("Frame"); progressTrack.Name = "ProgressTrack"; progressTrack.Size = UDim2.new(1, 0, 0, 2); progressTrack.Position = UDim2.new(0, 0, 0, 48); progressTrack.BackgroundColor3 = activeTheme.Border; progressTrack.BorderSizePixel = 0; progressTrack.Parent = mainFrame
    registerElement("Border", progressTrack, "BackgroundColor3")
    progressBar = Instance.new("Frame"); progressBar.Name = "ProgressBar"; progressBar.Size = UDim2.new(0, 0, 1, 0); progressBar.BackgroundColor3 = activeTheme.Accent; progressBar.BorderSizePixel = 0; progressBar.Parent = progressTrack
    registerElement("AccentBg", progressBar, "BackgroundColor3")

    local toastFrame = Instance.new("Frame"); toastFrame.Name = "Toast"; toastFrame.Size = UDim2.new(1, -28, 0, 32); toastFrame.Position = UDim2.new(0, 14, 1, 40); toastFrame.BackgroundColor3 = activeTheme.Surface; toastFrame.BorderSizePixel = 0; toastFrame.ZIndex = 20; toastFrame.Parent = mainFrame
    ...
    local toastLabel = Instance.new("TextLabel"); toastLabel.Size = UDim2.new(1, -16, 1, 0); toastLabel.Position = UDim2.new(0, 8, 0, 0); toastLabel.BackgroundTransparency = 1; toastLabel.Text = "Notification"; toastLabel.TextColor3 = activeTheme.TextPrimary; toastLabel.TextSize = 11; toastLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); toastLabel.ZIndex = 21; toastLabel.Parent = toastFrame
    registerElement("TextPrimary", toastLabel, "TextColor3")

    local toastSerial = 0
    showToast = function(message)
        toastLabel.Text = message; toastSerial = toastSerial + 1; local mySerial = toastSerial
        task.spawn(function()
            TweenService:Create(toastFrame, TWEEN_BOUNCE, { Position = UDim2.new(0, 14, 1, -44) }):Play()
            task.wait(2.2)
            if toastSerial == mySerial then TweenService:Create(toastFrame, TWEEN_QUICK, { Position = UDim2.new(0, 14, 1, 40) }):Play() end
        end)
    end

    local tabContainer = Instance.new("Frame"); tabContainer.Name = "Tabs"; tabContainer.Size = UDim2.new(1, 0, 0, 36); tabContainer.Position = UDim2.new(0, 0, 0, 50); tabContainer.BackgroundColor3 = activeTheme.Surface; tabContainer.BackgroundTransparency = 0.6; tabContainer.BorderSizePixel = 0; tabContainer.Parent = mainFrame
    ...
    local function createTabButton(name, targetPage, isDefault)
        local tabBtn = Instance.new("TextButton"); tabBtn.Size = UDim2.new(0, 72, 1, 0); tabBtn.BackgroundTransparency = 1; tabBtn.Text = name; tabBtn.TextColor3 = isDefault and activeTheme.Accent or activeTheme.TextSecondary; tabBtn.TextSize = 11; tabBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); tabBtn.Parent = tabContainer
        local indicator = Instance.new("Frame"); indicator.Size = UDim2.new(1, 0, 0, 2); indicator.Position = UDim2.new(0, 0, 1, -2); indicator.BackgroundColor3 = activeTheme.Accent; indicator.BackgroundTransparency = isDefault and 0 or 1; indicator.BorderSizePixel = 0; indicator.Parent = tabBtn
    ...
    local function createToggleRow(parent, name, labelText, activeColor, onToggled, order, defaultState)
        local row = Instance.new("Frame"); row.Name = name; row.LayoutOrder = order; row.Size = UDim2.new(1, 0, 0, 42); row.BackgroundColor3 = activeTheme.Surface; row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.Parent = parent
        ...
        local label = Instance.new("TextLabel"); label.Size = UDim2.new(1, -60, 1, 0); label.BackgroundTransparency = 1; label.Text = labelText; label.TextColor3 = activeTheme.TextPrimary; label.TextSize = 12; label.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); label.TextXAlignment = Enum.TextXAlignment.Left; label.Parent = row
        registerElement("TextPrimary", label, "TextColor3")

        local switchTrack = Instance.new("TextButton"); switchTrack.Name = "Track"; switchTrack.Size = UDim2.new(0, 42, 0, 22); switchTrack.Position = UDim2.new(1, -42, 0.5, -11); switchTrack.BackgroundColor3 = defaultState and activeColor or activeTheme.ToggleOff; switchTrack.BackgroundTransparency = 0.2; switchTrack.BorderSizePixel = 0; switchTrack.AutoButtonColor = false; switchTrack.Text = ""; switchTrack.Parent = row
        local trackCorner = Instance.new("UICorner"); trackCorner.CornerRadius = UDim.new(1, 0); trackCorner.Parent = switchTrack

        local switchKnob = Instance.new("Frame"); switchKnob.Name = "Knob"; switchKnob.Size = UDim2.new(0, 16, 0, 16); switchKnob.Position = defaultState and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8); switchKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255); switchKnob.BorderSizePixel = 0; switchKnob.Parent = switchTrack
        local knobCorner = Instance.new("UICorner"); knobCorner.CornerRadius = UDim.new(1, 0); knobCorner.Parent = switchKnob

        local isToggled = defaultState or false; local isInternalSetting = false
        ...
        row.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then setToggleState(not isToggled) end
        end)
    ...
    local function createSliderRow(parent, name, labelText, minVal, maxVal, defaultVal, formatStr, onValueChanged, order)
        local row = Instance.new("Frame"); row.Name = name; row.LayoutOrder = order; row.Size = UDim2.new(1, 0, 0, 48); row.BackgroundColor3 = activeTheme.Surface; row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.Parent = parent
        registerElement("Surface", row, "BackgroundColor3")
        local rowCorner = Instance.new("UICorner"); rowCorner.CornerRadius = UDim.new(0, 8); rowCorner.Parent = row
        local rowStroke = Instance.new("UIStroke"); rowStroke.Thickness = 1; rowStroke.Color = activeTheme.Border; rowStroke.Transparency = 0.5; rowStroke.Parent = row
        registerElement("Border", rowStroke, "Color")
        ...
        local fillBar = Instance.new("Frame"); local initRatio = math.clamp((defaultVal - minVal) / (maxVal - minVal), 0, 1); fillBar.Size = UDim2.new(initRatio, 0, 1, 0); fillBar.BackgroundColor3 = activeTheme.Accent; fillBar.BorderSizePixel = 0; fillBar.Parent = sliderBar
    ...
    local function createSectionHeader(parent, titleText, order)
        local secHeader = Instance.new("TextLabel"); secHeader.LayoutOrder = order; secHeader.Size = UDim2.new(1, 0, 0, 20); secHeader.BackgroundTransparency = 1; secHeader.Text = string.upper(titleText); secHeader.TextColor3 = activeTheme.Accent; secHeader.TextSize = 10; secHeader.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); secHeader.TextXAlignment = Enum.TextXAlignment.Left; secHeader.Parent = parent
        registerElement("AccentText", secHeader, "TextColor3"); return secHeader
    end

    telemetryCard = Instance.new("Frame"); telemetryCard.Name = "TelemetryCard"; telemetryCard.LayoutOrder = 1; telemetryCard.Size = UDim2.new(1, 0, 0, 44); telemetryCard.BackgroundColor3 = activeTheme.Surface; telemetryCard.BackgroundTransparency = 0.35; telemetryCard.BorderSizePixel = 0; telemetryCard.Parent = controlsPage
    ...
    nodeStatsLabel = Instance.new("TextLabel"); nodeStatsLabel.Size = UDim2.new(0.58, -4, 1, 0); nodeStatsLabel.Position = UDim2.new(0, 0, 0, 0); nodeStatsLabel.BackgroundTransparency = 1; nodeStatsLabel.Text = "Waypoints: 0 | Loop: 0/Inf"; nodeStatsLabel.TextColor3 = activeTheme.TextPrimary; nodeStatsLabel.TextSize = 11; nodeStatsLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); nodeStatsLabel.TextXAlignment = Enum.TextXAlignment.Left; nodeStatsLabel.Parent = telemetryCard
    registerElement("TextPrimary", nodeStatsLabel, "TextColor3")

    routeModeLabel = Instance.new("TextLabel"); routeModeLabel.Size = UDim2.new(0.42, -4, 1, 0); routeModeLabel.Position = UDim2.new(0.58, 4, 0, 0); routeModeLabel.BackgroundTransparency = 1; routeModeLabel.Text = "Mode: Loop (1.00x)"; routeModeLabel.TextColor3 = activeTheme.Accent; routeModeLabel.TextSize = 11; routeModeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); routeModeLabel.TextXAlignment = Enum.TextXAlignment.Right; routeModeLabel.Parent = telemetryCard
    registerElement("AccentText", routeModeLabel, "TextColor3")

    recordToggleControl = createToggleRow(controlsPage, "RecordToggle", "Record Route Path", activeTheme.RecordActive, function(state) if state then startRecording() else stopRecording() end end, 2)
    playToggleControl = createToggleRow(controlsPage, "PlayToggle", "Execute Playback Route", activeTheme.PlayActive, function(state) if state then startPlayback() else stopPlayback() end end, 3)

    local fileRow = Instance.new("Frame"); fileRow.Name = "FileRow"; fileRow.LayoutOrder = 4; fileRow.Size = UDim2.new(1, 0, 0, 42); fileRow.BackgroundColor3 = activeTheme.Surface; fileRow.BackgroundTransparency = 0.5; fileRow.BorderSizePixel = 0; fileRow.Parent = controlsPage
    ...
    local modeRow = Instance.new("Frame"); modeRow.Name = "ModeSetting"; modeRow.LayoutOrder = 2; modeRow.Size = UDim2.new(1, 0, 0, 42); modeRow.BackgroundColor3 = activeTheme.Surface; modeRow.BackgroundTransparency = 0.5; modeRow.BorderSizePixel = 0; modeRow.Parent = settingsPage
    ...
    local modeLabel = Instance.new("TextLabel"); modeLabel.Size = UDim2.new(1, -120, 1, 0); modeLabel.BackgroundTransparency = 1; modeLabel.Text = "Playback Route Mode"; modeLabel.TextColor3 = activeTheme.TextPrimary; modeLabel.TextSize = 12; modeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); modeLabel.TextXAlignment = Enum.TextXAlignment.Left; modeLabel.Parent = modeRow
    registerElement("TextPrimary", modeLabel, "TextColor3")

    local modeBtn = Instance.new("TextButton"); modeBtn.Size = UDim2.new(0, 100, 0, 26); modeBtn.Position = UDim2.new(1, -100, 0.5, -13); modeBtn.BackgroundColor3 = activeTheme.ToggleOff; modeBtn.BackgroundTransparency = 0.2; modeBtn.BorderSizePixel = 0; modeBtn.Text = Config.PlaybackMode; modeBtn.TextColor3 = activeTheme.Accent; modeBtn.TextSize = 11; modeBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); modeBtn.Parent = modeRow
    ...
    modeBtn.MouseButton1Click:Connect(function()
        modeIdx = (modeIdx % #modes) + 1; Config.PlaybackMode = modes[modeIdx]; modeBtn.Text = Config.PlaybackMode; updateTelemetry(nil, #Waypoints)
    end)

    createSliderRow(settingsPage, "LoopSlider", "Loop Count Limit", 0, 200, Config.TargetLoops, "Loops: %d", function(val) Config.TargetLoops = math.floor(val); updateTelemetry(nil, #Waypoints) end, 3)
    createSliderRow(settingsPage, "SpeedSlider", "Playback Speed", 0.5, 3.0, Config.SpeedMultiplier, "%.2fx", function(val) Config.SpeedMultiplier = math.floor(val * 100) / 100; updateTelemetry(nil, #Waypoints) end, 4)

    createSectionHeader(settingsPage, "Hardware Bridge & Integration", 5)
    createToggleRow(settingsPage, "BridgeToggle", "External Macro Bridge (.NET)", activeTheme.PlayActive, function(state) Config.UseExternalBridge = state; showToast("Hardware Bridge: " .. (state and "Enabled" or "Disabled")) end, 6, Config.UseExternalBridge)

    createSectionHeader(settingsPage, "Display & Visualizer", 7)
    createToggleRow(settingsPage, "StatusHUDToggle", "Persistent Status Bar (Draggable)", activeTheme.Accent, function(state) Config.StatusHUDEnabled = state; statusHUD.Visible = state end, 8, Config.StatusHUDEnabled)
    createToggleRow(settingsPage, "RealtimeVisToggle", "Realtime Visual Rendering (While Recording)", activeTheme.Accent, function(state) Config.RealtimeVisualizer = state end, 9, Config.RealtimeVisualizer)
    createToggleRow(settingsPage, "WaypointLabelsToggle", "3D Waypoint Tags ([E], [Start], [End])", activeTheme.Accent, function(state) Config.WaypointLabelsEnabled = state; renderVisualPath(Waypoints) end, 10, Config.WaypointLabelsEnabled)
    createToggleRow(settingsPage, "VisualizerToggle", "3D Workspace Visualizer", activeTheme.Accent, function(state) Config.VisualizerEnabled = state; if state then renderVisualPath(Waypoints) else clearVisuals() end end, 11, Config.VisualizerEnabled)
    createSliderRow(settingsPage, "OrbOpacitySlider", "Visualizer Orb Opacity", 0.0, 0.8, Config.VisualizerOpacity, "Opacity: %.2f", function(val) Config.VisualizerOpacity = math.floor(val * 100) / 100; renderVisualPath(Waypoints) end, 12)
    ...
    local function createKeybindRow(labelText, actionKeyName, order)
        local row = Instance.new("Frame"); row.Name = "Hotkey_" .. actionKeyName; row.LayoutOrder = order; row.Size = UDim2.new(1, 0, 0, 42); row.BackgroundColor3 = activeTheme.Surface; row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.Parent = settingsPage
        registerElement("Surface", row, "BackgroundColor3")
        ...
        local kLabel = Instance.new("TextLabel"); kLabel.Size = UDim2.new(1, -120, 1, 0); kLabel.BackgroundTransparency = 1; kLabel.Text = labelText; kLabel.TextColor3 = activeTheme.TextPrimary; kLabel.TextSize = 12; kLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); kLabel.TextXAlignment = Enum.TextXAlignment.Left; kLabel.Parent = row
        registerElement("TextPrimary", kLabel, "TextColor3")

        local kBtn = Instance.new("TextButton"); kBtn.Size = UDim2.new(0, 100, 0, 26); kBtn.Position = UDim2.new(1, -100, 0.5, -13); kBtn.BackgroundColor3 = activeTheme.ToggleOff; kBtn.BackgroundTransparency = 0.2; kBtn.BorderSizePixel = 0; kBtn.AutoButtonColor = false; kBtn.Text = Keybinds[actionKeyName].Name; kBtn.TextColor3 = activeTheme.TextPrimary; kBtn.TextSize = 11; kBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); kBtn.Parent = row
        ...
        kBtn.MouseButton1Click:Connect(function()
            ...
            activeBindingKey = actionKeyName; kBtn.Text = "Press Key..."; kBtn.TextColor3 = activeTheme.Accent
        end)
    ...
    local inputConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            elseif input.KeyCode == Keybinds.ToggleRecord then if recordToggleControl and typeof(recordToggleControl.Get) == "function" then recordToggleControl.Set(not recordToggleControl.Get()) end
            elseif input.KeyCode == Keybinds.TogglePlay then if playToggleControl and typeof(playToggleControl.Get) == "function" then playToggleControl.Set(not playToggleControl.Get()) end
            elseif input.KeyCode == Keybinds.UndoNode then undoLastNode() end
        end
    end)
    ...
    local function applyTheme(themeName, customPalette)
        currentThemeName = themeName; activeTheme = customPalette or THEMES[themeName]
        ...
        if activeTheme.GradientEnabled then
            bgGradient.Enabled = true; bgGradient.Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, activeTheme.Background), ColorSequenceKeypoint.new(1, activeTheme.GradientColor or activeTheme.Accent) })
        else bgGradient.Enabled = false end
    ...
    if readfile and isfile and isfile("ManniskaFarm_CustomTheme.json") then
        pcall(function()
            local raw = readfile("ManniskaFarm_CustomTheme.json"); local dec = HttpService:JSONDecode(raw)
            if typeof(dec) == "table" and dec.Hue then CustomThemeData.Hue = dec.Hue; CustomThemeData.Saturation = dec.Saturation; CustomThemeData.Brightness = dec.Brightness; CustomThemeData.GradientEnabled = (dec.GradientEnabled == true); task.defer(updateCustomThemeFromSliders) end
        end)
    end

    -- --- ADVANCED TAB ---
    createSectionHeader(advancedPage, "Humanization & Randomization", 1)
    createToggleRow(advancedPage, "MicroRandToggle", "Micro Randomization (Human Timing & Path)", activeTheme.Accent, function(state) Config.MicroRandomization = state; showToast("Micro Randomization: " .. (state and "Enabled" or "Disabled")) end, 2, Config.MicroRandomization)
    ...
    createToggleRow(advancedPage, "SegmentToggle", "Enable Waypoint Segment Loop", activeTheme.Accent, function(state) Config.SegmentLooping = state end, 8, Config.SegmentLooping)
    createSliderRow(advancedPage, "SegStartSlider", "Segment Start Node", 1, 100, Config.SegmentStart, "Start: %d", function(val) Config.SegmentStart = math.floor(val) end, 9)
    createSliderRow(advancedPage, "SegEndSlider", "Segment End Node", 1, 100, Config.SegmentEnd, "End: %d", function(val) Config.SegmentEnd = math.floor(val) end, 10)
    createSliderRow(advancedPage, "SegRepeatSlider", "Segment Repeats", 1, 10, Config.SegmentRepeats, "Repeats: %dx", function(val) Config.SegmentRepeats = math.floor(val) end, 11)

    createSectionHeader(advancedPage, "Rejoin Persistence", 12)
    createToggleRow(advancedPage, "AutoRejoinToggle", "Auto-Execute on Rejoin", activeTheme.Accent, function(state) Config.AutoRejoin = state; if writefile then writefile("ManniskaFarm_AutoRun.json", HttpService:JSONEncode({ Enabled = state, Route = Config.FileName })) end end, 13, Config.AutoRejoin)
    ...
    local radarActionRow = Instance.new("Frame"); radarActionRow.Name = "RadarActionSetting"; radarActionRow.LayoutOrder = 4; radarActionRow.Size = UDim2.new(1, 0, 0, 42); radarActionRow.BackgroundColor3 = activeTheme.Surface; radarActionRow.BackgroundTransparency = 0.5; radarActionRow.BorderSizePixel = 0; radarActionRow.Parent = safetyPage
    registerElement("Surface", radarActionRow, "BackgroundColor3")
    local rCorner = Instance.new("UICorner"); rCorner.CornerRadius = UDim.new(0, 8); rCorner.Parent = radarActionRow
    local rStroke = Instance.new("UIStroke"); rStroke.Thickness = 1; rStroke.Color = activeTheme.Border; rStroke.Transparency = 0.5; rStroke.Parent = radarActionRow
    registerElement("Border", rStroke, "Color")
    local rPadding = Instance.new("UIPadding"); rPadding.PaddingLeft = UDim.new(0, 14); rPadding.PaddingRight = UDim.new(0, 14); rPadding.Parent = radarActionRow

    local rLabel = Instance.new("TextLabel"); rLabel.Size = UDim2.new(1, -120, 1, 0); rLabel.BackgroundTransparency = 1; rLabel.Text = "Radar Trigger Action"; rLabel.TextColor3 = activeTheme.TextPrimary; rLabel.TextSize = 12; rLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); rLabel.TextXAlignment = Enum.TextXAlignment.Left; rLabel.Parent = radarActionRow
    registerElement("TextPrimary", rLabel, "TextColor3")

    local rBtn = Instance.new("TextButton"); rBtn.Size = UDim2.new(0, 100, 0, 26); rBtn.Position = UDim2.new(1, -100, 0.5, -13); rBtn.BackgroundColor3 = activeTheme.ToggleOff; rBtn.BackgroundTransparency = 0.2; rBtn.BorderSizePixel = 0; rBtn.Text = Config.RadarAction; rBtn.TextColor3 = activeTheme.Accent; rBtn.TextSize = 11; rBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); rBtn.Parent = radarActionRow
    registerElement("ToggleOff", rBtn, "BackgroundColor3")
    local rBtnCorner = Instance.new("UICorner"); rBtnCorner.CornerRadius = UDim.new(0, 6); rBtnCorner.Parent = rBtn
    ...
    createToggleRow(safetyPage, "LeaveOnDeathToggle", "Emergency Leave on Death", Color3.fromRGB(240, 70, 70), function(state) Config.EmergencyLeaveOnDeath = state; showToast("Leave on Death: " .. (state and "Enabled" or "Disabled")) end, 6, Config.EmergencyLeaveOnDeath)
    ...
    local terminateBtn = Instance.new("TextButton"); terminateBtn.Name = "TerminateButton"; terminateBtn.LayoutOrder = 10; terminateBtn.Size = UDim2.new(1, 0, 0, 38); terminateBtn.BackgroundColor3 = Color3.fromRGB(45, 20, 24); terminateBtn.BackgroundTransparency = 0.3; terminateBtn.BorderSizePixel = 0; terminateBtn.AutoButtonColor = false; terminateBtn.Text = "Terminate & Unload Script"; terminateBtn.TextColor3 = Color3.fromRGB(255, 95, 95); terminateBtn.TextSize = 12; terminateBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); terminateBtn.Parent = safetyPage
    ...
    terminateBtn.MouseEnter:Connect(function() TweenService:Create(terminateBtn, TWEEN_QUICK, { BackgroundColor3 = Color3.fromRGB(200, 40, 40), TextColor3 = Color3.fromRGB(255, 255, 255) }):Play() end)
    terminateBtn.MouseLeave:Connect(function() TweenService:Create(terminateBtn, TWEEN_QUICK, { BackgroundColor3 = Color3.fromRGB(45, 20, 24), TextColor3 = Color3.fromRGB(255, 95, 95) }):Play() end)
    terminateBtn.MouseButton1Click:Connect(function() terminateProcess() end)

    local resizeHandle = Instance.new("TextButton"); resizeHandle.Name = "ResizeHandle"; resizeHandle.Size = UDim2.new(0, 18, 0, 18); resizeHandle.Position = UDim2.new(1, -18, 1, -18); resizeHandle.BackgroundTransparency = 1; resizeHandle.Text = "◢"; resizeHandle.TextColor3 = activeTheme.TextSecondary; resizeHandle.TextSize = 11; resizeHandle.Parent = mainFrame
    registerElement("TextSecondary", resizeHandle, "TextColor3")

    local dragging, dragStartPos, frameStartPos = false, nil, nil
    header.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStartPos = input.Position; frameStartPos = mainFrame.Position
    ...
    resizeHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            resizing = true; resizeStartPos = input.Position; frameStartSize = mainFrame.AbsoluteSize
        ...
        if isRecording then
            if input.KeyCode == Enum.KeyCode.E and holdStartTick then
                ...
                table.insert(Waypoints, {
                    ...
                    interClickDelay = 0.2, delay = now - lastWaypointTime
                })
            ...
            elseif Config.RecordMouseClicks and input.UserInputType == Enum.UserInputType.MouseButton1 and mb1StartTick then
                ...
                table.insert(Waypoints, {
                    ...
                    jump = false, pauseDuration = 0, speed = h and h.WalkSpeed or 16, delay = now - lastWaypointTime
                })
                lastWaypointTime = now
                updateTelemetry(nil, #Waypoints)
                if Config.RealtimeVisualizer then renderVisualPath(Waypoints) end
...
startRecording = function()
    ...
    table.insert(Waypoints, { pos = initialPos, jump = false, pauseDuration = 0, speed = hum and hum.WalkSpeed or 16, isSprinting = hum and (hum.WalkSpeed > 17) or false, delay = 0.08 })
    updateTelemetry(nil, #Waypoints)
    if Config.RealtimeVisualizer then renderVisualPath(Waypoints) end
        ...
        while isRecording do
            ...
            if pos then
                ...
                if distMoved > 0.4 or stampedJump then
                    standingStillTime = 0
                    table.insert(Waypoints, { pos = pos, jump = stampedJump, pauseDuration = 0, speed = h and h.WalkSpeed or 16, isSprinting = h and (h.WalkSpeed > 17) or false, delay = now - lastWaypointTime })
                    lastRecordedPos = pos
                    lastWaypointTime = now
                    updateTelemetry(nil, #Waypoints)
                    if Config.RealtimeVisualizer then renderVisualPath(Waypoints) end
...
task.spawn(function()
    ...
    if success and typeof(dec) == "table" and dec.Enabled and dec.Route then
        ...
        if root and root:IsA("BasePart") then
            ...
            if playToggleControl and typeof(playToggleControl.Set) == "function" then playToggleControl.Set(true, false) else startPlayback() end
...
print("🚀 Autofarm V12.6 (Surgical Precision Patch) Loaded.")
