...
-- Visualizer Setup & Memory Pooling
...
local function getVisBeam(idx)
    ...
    visualizerPool.Beams[idx] = beam
    return beam
end

local function getVisLabel(idx, node)
    if visualizerPool.Labels[idx] then 
        visualizerPool.Labels[idx].Billboard.Adornee = node
        return visualizerPool.Labels[idx]
    end
    ...
    local lblObj = { Billboard = billboard, Tag = tag }
    visualizerPool.Labels[idx] = lblObj
    return lblObj
end

clearVisuals = function()
    for _, node in ipairs(visualizerPool.Nodes) do 
        node.Transparency = 1
        node.CFrame = CFrame.new(0, -5000, 0)
    end
    for _, beam in ipairs(visualizerPool.Beams) do 
        beam.Transparency = 1
        beam.CFrame = CFrame.new(0, -5000, 0)
    end
    for _, lbl in ipairs(visualizerPool.Labels) do 
        lbl.Billboard.Enabled = false 
    end
    activeNodeHalo.Transparency = 1
end
...
print("🚀 Autofarm V12.5 (Strict Timing & Pooled Memory) Loaded.")
