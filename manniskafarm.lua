-- =====================================================================
--  MANNISKAFARM V14.5 - DOUBLE-CHECKED HEAP ARCHITECTURE
-- =====================================================================
...
-- =====================================================================
--  MASTER CORE DEFINITIONS & DUMMY STUBS
-- =====================================================================
local Core = {
...
-- DUMMY STUBS: Prevents ANY possibility of calling a nil UI function early
Core.showToast = function(msg) warn("[ManniskaFarm] " .. tostring(msg)) end
Core.updateStateBadge = function(name, color) end
Core.triggerErrorModal = function(code, desc) warn("[ManniskaFarm Error] " .. tostring(code) .. ": " .. tostring(desc)) end
Core.UI.setGuiVisible = function(vis) end
...
Core.stopRecording = function()
    Core.isRecording = false; Core.jumpTriggered = false; Core.holdStartTick = nil; Core.mb1StartTick = nil
    if Core.scriptConnections.jump then Core.scriptConnections.jump:Disconnect(); Core.scriptConnections.jump = nil end
    if Core.scriptConnections.key then Core.scriptConnections.key:Disconnect(); Core.scriptConnections.key = nil end
    if Core.scriptConnections.keyEnd then Core.scriptConnections.keyEnd:Disconnect(); Core.scriptConnections.keyEnd = nil end
    if Core.scriptConnections.state then Core.scriptConnections.state:Disconnect(); Core.scriptConnections.state = nil end
...
Core.startRecording = function()
    ...
    Core.Waypoints = {}
    Core.clearVisuals()
    Core.isRecording = true; Core.jumpTriggered = false; Core.holdStartTick = nil; Core.mb1StartTick = nil; Core.currentLoopCount = 0; Core.lastWaypointTime = tick()
...
titleLabel.Size = UDim2.new(0, 145, 1, 0); titleLabel.BackgroundTransparency = 1; titleLabel.Text = "MANNISKAFARM V14.5"; titleLabel.TextColor3 = activeTheme.TextPrimary; titleLabel.TextSize = 13; titleLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); titleLabel.TextXAlignment = Enum.TextXAlignment.Left; titleLabel.Parent = titleTopRow
...
Core.UI.setGuiVisible = setGuiVisible -- EXPOSED TO CORE
...
local inputConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if Core.activeBindingKey and input.UserInputType == Enum.UserInputType.Keyboard then
        Keybinds[Core.activeBindingKey] = input.KeyCode
        if Core.activeBindingKey == "ToggleMenu" then bindMenu.Button.Text = input.KeyCode.Name; bindMenu.Button.TextColor3 = activeTheme.TextPrimary end
        if Core.activeBindingKey == "ToggleRecord" then bindRec.Button.Text = input.KeyCode.Name; bindRec.Button.TextColor3 = activeTheme.TextPrimary end
        if Core.activeBindingKey == "TogglePlay" then bindPlay.Button.Text = input.KeyCode.Name; bindPlay.Button.TextColor3 = activeTheme.TextPrimary end
        if Core.activeBindingKey == "UndoNode" then bindUndo.Button.Text = input.KeyCode.Name; bindUndo.Button.TextColor3 = activeTheme.TextPrimary end
    ...
    if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard then
        if input.KeyCode == Keybinds.ToggleMenu then 
            if Core.UI.setGuiVisible and Core.UI.mainFrame then Core.UI.setGuiVisible(not Core.UI.mainFrame.Visible) end
...
local function applyTheme(themeName, customPalette)
    ...
    for _, item in ipairs(Core.themeElements.Background) do item.Object[item.Property] = activeTheme.Background end
    for _, item in ipairs(Core.themeElements.Surface) do item.Object[item.Property] = activeTheme.Surface end
    for _, item in ipairs(Core.themeElements.Border) do item.Object[item.Property] = activeTheme.Border end
    for _, item in ipairs(Core.themeElements.AccentText) do item.Object[item.Property] = activeTheme.Accent end
    for _, item in ipairs(Core.themeElements.AccentBg) do item.Object[item.Property] = activeTheme.Accent end
    for _, item in ipairs(Core.themeElements.TextPrimary) do item.Object[item.Property] = activeTheme.TextPrimary end
    for _, item in ipairs(Core.themeElements.TextSecondary) do item.Object[item.Property] = activeTheme.TextSecondary end
    for _, item in ipairs(Core.themeElements.ToggleOff) do item.Object[item.Property] = activeTheme.ToggleOff end
...
task.spawn(function()
    ...
    if success and typeof(dec) == "table" and dec.Enabled and dec.Route then
        ...
        if root and root:IsA("BasePart") then
            ...
            if Core.showToast then Core.showToast("Auto-Rejoin: Resumed route " .. dec.Route) end
        end
    end
end)
...
print("🚀 Autofarm V14.5 (Heap Verified) Loaded.")
