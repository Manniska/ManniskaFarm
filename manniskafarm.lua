...
local THEMES = {
    ...
    ["Cyber Purple"] = {
        Background = Color3.fromRGB(16, 12, 24), Surface = Color3.fromRGB(26, 20, 38),
        Border = Color3.fromRGB(60, 42, 88), Accent = Color3.fromRGB(168, 85, 247),
        RecordActive = Color3.fromRGB(240, 70, 70), PlayActive = Color3.fromRGB(46, 204, 113),
        ToggleOff = Color3.fromRGB(40, 32, 58), TextPrimary = Color3.fromRGB(248, 245, 252),
        TextSecondary = Color3.fromRGB(165, 150, 185), GradientEnabled = true, GradientColor = Color3.fromRGB(90, 20, 140)
    }
}
...
-- =====================================================================
--  ADORNMENT VISUALIZER OVERHAUL (ZERO LAG)
-- =====================================================================
...
do
    ...
    bgGradient.Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, activeTheme.Background), ColorSequenceKeypoint.new(1, activeTheme.GradientColor or activeTheme.Background) })
    ...
    titleTopRow.Size = UDim2.new(1, -110, 1, 0)
    titleTopRow.Position = UDim2.new(0, 14, 0, 0)
    titleTopRow.BackgroundTransparency = 1
    titleTopRow.Parent = header
    ...
    headerBtnGroup.Position = UDim2.new(1, -74, 0, 0)
    ...
    local function createToggleRow(parent, name, labelText, activeColor, onToggled, order, defaultState)
        ...
        label.Text = labelText; label.TextColor3 = activeTheme.TextPrimary; label.TextSize = 12
        label.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); label.TextXAlignment = Enum.TextXAlignment.Left; label.Parent = row

        local switchTrack = Instance.new("TextButton")
        switchTrack.Size = UDim2.new(0, 42, 0, 22); switchTrack.Position = UDim2.new(1, -42, 0.5, -11)
        switchTrack.BackgroundColor3 = defaultState and activeColor or activeTheme.ToggleOff
        switchTrack.BackgroundTransparency = 0.2; switchTrack.BorderSizePixel = 0; switchTrack.AutoButtonColor = false; switchTrack.Text = ""; switchTrack.Parent = row
        local trackCorner = Instance.new("UICorner"); trackCorner.CornerRadius = UDim.new(1, 0); trackCorner.Parent = switchTrack

        local switchKnob = Instance.new("Frame")
        switchKnob.Size = UDim2.new(0, 16, 0, 16); switchKnob.Position = defaultState and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
        switchKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255); switchKnob.BorderSizePixel = 0; switchKnob.Parent = switchTrack
    ...
    local function createActionButton(parent, labelText, order, onClick, customColor)
        ...
        btn.LayoutOrder = order; btn.Size = UDim2.new(1, 0, 0, 36)
        btn.BackgroundColor3 = customColor or activeTheme.Surface
        btn.BackgroundTransparency = customColor and 0.25 or 0.4
        btn.BorderSizePixel = 0; btn.AutoButtonColor = false
        btn.Text = labelText; btn.TextColor3 = customColor and Color3.fromRGB(255, 255, 255) or activeTheme.TextSecondary
        btn.TextSize = 12; btn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); btn.Parent = parent
    ...
    local function createSliderRow(parent, name, labelText, minVal, maxVal, defaultVal, formatStr, onValueChanged, order)
        ...
        row.BackgroundColor3 = activeTheme.Surface; row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.Parent = parent
        local rowCorner = Instance.new("UICorner"); rowCorner.CornerRadius = UDim.new(0, 8); rowCorner.Parent = row
        local rowStroke = Instance.new("UIStroke"); rowStroke.Thickness = 1; rowStroke.Color = activeTheme.Border; rowStroke.Transparency = 0.5; rowStroke.Parent = row
        local rowPadding = Instance.new("UIPadding"); rowPadding.PaddingLeft = UDim.new(0, 14); rowPadding.PaddingRight = UDim.new(0, 14); rowPadding.Parent = row

        local titleLbl = Instance.new("TextLabel")
        titleLbl.Size = UDim2.new(0.6, 0, 0, 20); titleLbl.Position = UDim2.new(0, 0, 0, 6)
        titleLbl.BackgroundTransparency = 1; titleLbl.Text = labelText; titleLbl.TextColor3 = activeTheme.TextPrimary
        titleLbl.TextSize = 12; titleLbl.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); titleLbl.TextXAlignment = Enum.TextXAlignment.Left; titleLbl.Parent = row

        local valLbl = Instance.new("TextLabel")
        valLbl.Size = UDim2.new(0.4, 0, 0, 20); valLbl.Position = UDim2.new(0.6, 0, 0, 6)
        valLbl.BackgroundTransparency = 1; valLbl.Text = (formatStr == "Loops: %d" and defaultVal == 0) and "Loops: Inf" or string.format(formatStr, defaultVal)
        valLbl.TextColor3 = activeTheme.Accent; valLbl.TextSize = 12; valLbl.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); valLbl.TextXAlignment = Enum.TextXAlignment.Right; valLbl.Parent = row

        local sliderBar = Instance.new("Frame")
        sliderBar.Size = UDim2.new(1, 0, 0, 6); sliderBar.Position = UDim2.new(0, 0, 1, -14)
        sliderBar.BackgroundColor3 = activeTheme.ToggleOff; sliderBar.BorderSizePixel = 0; sliderBar.Parent = row
    ...
    local function createSectionHeader(parent, titleText, order)
        ...
        secHeader.LayoutOrder = order; secHeader.Size = UDim2.new(1, 0, 0, 20)
        secHeader.BackgroundTransparency = 1; secHeader.Text = string.upper(titleText)
        secHeader.TextColor3 = activeTheme.Accent; secHeader.TextSize = 10
        secHeader.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); secHeader.TextXAlignment = Enum.TextXAlignment.Left; secHeader.Parent = parent
        return secHeader
    end

    local telemetryCard = Instance.new("Frame")
    telemetryCard.LayoutOrder = 1; telemetryCard.Size = UDim2.new(1, 0, 0, 44)
    telemetryCard.BackgroundColor3 = activeTheme.Surface; telemetryCard.BackgroundTransparency = 0.35; telemetryCard.BorderSizePixel = 0; telemetryCard.Parent = controlsPage
    ...
    nodeStatsLabel.Size = UDim2.new(0.58, -4, 1, 0); nodeStatsLabel.Position = UDim2.new(0, 0, 0, 0)
    nodeStatsLabel.BackgroundTransparency = 1; nodeStatsLabel.Text = "Waypoints: 0 | Loop: 0/Inf"
    nodeStatsLabel.TextColor3 = activeTheme.TextPrimary; nodeStatsLabel.TextSize = 11
    nodeStatsLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); nodeStatsLabel.TextXAlignment = Enum.TextXAlignment.Left; nodeStatsLabel.Parent = telemetryCard

    routeModeLabel = Instance.new("TextLabel")
    routeModeLabel.Size = UDim2.new(0.42, -4, 1, 0); routeModeLabel.Position = UDim2.new(0.58, 4, 0, 0)
    routeModeLabel.BackgroundTransparency = 1; routeModeLabel.Text = "Mode: Loop (1.00x)"
    routeModeLabel.TextColor3 = activeTheme.Accent; routeModeLabel.TextSize = 11
    routeModeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); routeModeLabel.TextXAlignment = Enum.TextXAlignment.Right; routeModeLabel.Parent = telemetryCard
    ...
    fileRow.LayoutOrder = 4; fileRow.Size = UDim2.new(1, 0, 0, 42)
    fileRow.BackgroundColor3 = activeTheme.Surface; fileRow.BackgroundTransparency = 0.5; fileRow.BorderSizePixel = 0; fileRow.Parent = controlsPage
    ...
    fileInput.Size = UDim2.new(1, -156, 1, 0); fileInput.Position = UDim2.new(0, 10, 0, 0)
    fileInput.BackgroundTransparency = 1; fileInput.Text = "ManniskaFarm_Route.json"; fileInput.PlaceholderText = "RouteFileName.json"
    fileInput.TextColor3 = activeTheme.TextPrimary; fileInput.TextSize = 12; fileInput.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); fileInput.TextXAlignment = Enum.TextXAlignment.Left; fileInput.ClearTextOnFocus = false; fileInput.Parent = fileRow

    local saveBtn = Instance.new("TextButton")
    saveBtn.Size = UDim2.new(0, 64, 0, 26); saveBtn.Position = UDim2.new(1, -138, 0.5, -13)
    saveBtn.BackgroundColor3 = activeTheme.Border; saveBtn.BackgroundTransparency = 0.3; saveBtn.BorderSizePixel = 0
    saveBtn.Text = "Save"; saveBtn.TextColor3 = activeTheme.TextPrimary; saveBtn.TextSize = 11; saveBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); saveBtn.Parent = fileRow
    ...
    loadBtn.Size = UDim2.new(0, 64, 0, 26); loadBtn.Position = UDim2.new(1, -68, 0.5, -13)
    loadBtn.BackgroundColor3 = activeTheme.Accent; loadBtn.BackgroundTransparency = 0.2; loadBtn.BorderSizePixel = 0
    loadBtn.Text = "Load"; loadBtn.TextColor3 = Color3.fromRGB(255, 255, 255); loadBtn.TextSize = 11; loadBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); loadBtn.Parent = fileRow
    ...
    dropdownCard.LayoutOrder = 5; dropdownCard.Size = UDim2.new(1, 0, 0, 72)
    dropdownCard.BackgroundColor3 = activeTheme.Surface; dropdownCard.BackgroundTransparency = 0.6; dropdownCard.BorderSizePixel = 0; dropdownCard.Parent = controlsPage
    ...
    ddList.Size = UDim2.new(1, -12, 1, -8); ddList.Position = UDim2.new(0, 6, 0, 4)
    ddList.BackgroundTransparency = 1; ddList.BorderSizePixel = 0; ddList.ScrollBarThickness = 2; ddList.ScrollBarImageColor3 = activeTheme.Accent
    ddList.CanvasSize = UDim2.new(0, 0, 0, 0); ddList.AutomaticCanvasSize = Enum.AutomaticSize.Y; ddList.Parent = dropdownCard
    ...
    local function refreshFileList()
        ...
        for _, path in ipairs(files) do
            if string.find(path, ".json") then
                ...
                fBtn.Size = UDim2.new(1, 0, 0, 22); fBtn.BackgroundColor3 = activeTheme.ToggleOff; fBtn.BackgroundTransparency = 0.4; fBtn.BorderSizePixel = 0
                fBtn.Text = "   📁 " .. cleanName; fBtn.TextColor3 = activeTheme.TextPrimary; fBtn.TextSize = 10
                fBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); fBtn.TextXAlignment = Enum.TextXAlignment.Left; fBtn.Parent = ddList
    ...
    modeRow.LayoutOrder = 2; modeRow.Size = UDim2.new(1, 0, 0, 42)
    modeRow.BackgroundColor3 = activeTheme.Surface; modeRow.BackgroundTransparency = 0.5; modeRow.BorderSizePixel = 0; modeRow.Parent = settingsPage
    ...
    modeLabel.Size = UDim2.new(1, -120, 1, 0); modeLabel.BackgroundTransparency = 1
    modeLabel.Text = "Playback Route Mode"; modeLabel.TextColor3 = activeTheme.TextPrimary; modeLabel.TextSize = 12
    modeLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); modeLabel.TextXAlignment = Enum.TextXAlignment.Left; modeLabel.Parent = modeRow

    local modeBtn = Instance.new("TextButton")
    modeBtn.Size = UDim2.new(0, 100, 0, 26); modeBtn.Position = UDim2.new(1, -100, 0.5, -13)
    modeBtn.BackgroundColor3 = activeTheme.ToggleOff; modeBtn.BackgroundTransparency = 0.2; modeBtn.BorderSizePixel = 0
    modeBtn.Text = Config.PlaybackMode; modeBtn.TextColor3 = activeTheme.Accent; modeBtn.TextSize = 11
    modeBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold); modeBtn.Parent = modeRow
    ...
    createToggleRow(settingsPage, "WaypointLabelsToggle", "3D Waypoint Tags", activeTheme.Accent, function(state) Config.WaypointLabelsEnabled = state; renderVisualPath(Waypoints) end, 10, Config.WaypointLabelsEnabled)
    ...
    local function createKeybindRow(labelText, actionKeyName, order)
        local row = Instance.new("Frame")
        row.LayoutOrder = order; row.Size = UDim2.new(1, 0, 0, 42)
        row.BackgroundColor3 = activeTheme.Surface; row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.Parent = settingsPage
        ...
        kLabel.Size = UDim2.new(1, -120, 1, 0); kLabel.BackgroundTransparency = 1
        kLabel.Text = labelText; kLabel.TextColor3 = activeTheme.TextPrimary; kLabel.TextSize = 12
        kLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium); kLabel.TextXAlignment = Enum.TextXAlignment.Left; kLabel.Parent = row

        local kBtn = Instance.new("TextButton")
        kBtn.Size = UDim2.new(0, 100, 0, 26); kBtn.Position = UDim2.new(1, -100, 0.5, -13)
        kBtn.BackgroundColor3 = activeTheme.ToggleOff; kBtn.BackgroundTransparency = 0.2; kBtn.BorderSizePixel = 0; kBtn.AutoButtonColor = false
        kBtn.Text = Keybinds[actionKeyName].Name; kBtn.TextColor3 = activeTheme.TextPrimary; kBtn.TextSize = 11
        kBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); kBtn.Parent = row
    ...
    terminateBtn.LayoutOrder = 10; terminateBtn.Size = UDim2.new(1, 0, 0, 38)
    terminateBtn.BackgroundColor3 = Color3.fromRGB(45, 20, 24); terminateBtn.BackgroundTransparency = 0.3; terminateBtn.BorderSizePixel = 0
    terminateBtn.Text = "Terminate & Unload Script"; terminateBtn.TextColor3 = Color3.fromRGB(255, 95, 95); terminateBtn.TextSize = 12
    terminateBtn.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold); terminateBtn.Parent = safetyPage
    ...
    resizeHandle.BackgroundTransparency = 1; resizeHandle.Text = "◢"; resizeHandle.TextColor3 = activeTheme.TextSecondary; resizeHandle.Parent = mainFrame
...
sendExternalMacroCommand = function(key, durationMs)
